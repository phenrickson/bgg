...
day_windows = c(30, 60, 90, 180, 360)
...
```{r predict one game from the validation set, fig.height=8, fig.width=10}
...
pred_validation %>%
        ...
        geom_line(data = games_validation_lags %>%
                          filter(name %in% samp) %>%
                          filter(days_since_release < 365) %>%
                          mutate(days = days_since_release),
                          aes(x=days,
                              by = name,
                              y = exp(users_rated)))+
        geom_vline(xintercept = c(day_windows),
                   linetype = 'dotted')+
        theme(legend.position = 'none')
