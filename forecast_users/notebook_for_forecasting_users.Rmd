...
This notebook is for working with historical time series data from boardgamegeek. Can we forecast the number of ratings a game will get up to two years out?
...
Our main dataset for analysis is daily table of game rankings and user ratings the end of 2016 to present. We'll additionally pull down an active table of games currently stand on BGG (as of the last time we refreshed it).
...
So, we now have a pretty tidy dataset of games from 2016 onwards. We can look at games over this time period to show what this dataset looks like.
