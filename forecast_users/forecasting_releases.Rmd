...
We start tracking some games *well* after they first were released, such as Knucklebones, which entered the dataset in 2021 but is listed as coming out in 3000 BC.
...
Once a game enters the dataset, however, we can plot the trajectory of each game on user ratings, average rating, geek rating, and bgg rank.
...
Our goal is to forecast the trajectory of each game, focusing particularly on user ratings.
...
Let's create a dataset with only games that were released during this time period. We'll then split our data into a training and validation set. We'll then create a recipe to determine the key features of games we aim to use in forecasting. 
...
games_release_full<-games_release %>% 
        ...
        left_join(., active_games %>% 
                          select(-baverage,
                                 ...
                                 -numcomments,
                                 -timestamp),
                  by = c("game_id", "name", "yearpublished")) # add in info from active table, such as player info, etc
...
# training set with mechanic, category features
games_release_train = games_release_full %>%
        ...
        left_join(., games_train %>%
                          select(game_id,
                                 ...
                                 # starts_with("art_")),
                  by = c("game_id")) %>%
        filter(cat_collectible_components !=1 &
                       cat_expansion_for_basegame != 1) %>%
        mutate_at(vars(starts_with("mech_")),
                  ~ replace_na(., 0)) %>%
        mutate_at(vars(starts_with("cat_")),
                  ~ replace_na(., 0))

# remove specific categories that count expansions
rm(games_release)
gc()

```

How many games do we have from each year in our traiing set?
...
The clock for a game starts the moment (roughly) it hits 30 user ratings - starting from this point, we want to forecast a game's user ratings and average bgg ratings outcomes in 7 days, 30 days, 90 days, 180 days, and 365 days.

We have a couple of options for doing this. We can do a rolling, multi-fstep orecast, where we predict an outcome at time $T_1$ using features at $T$ and then use our forecasts from $T+1$ to serve as the input for forecasting time T+2, and so on.
...
What is the autocorrelation for user ratings and bgg average ratings? That is, how similar are these variables to their lagged values?
...
games_release_train %>%
  as.data.frame() %>%
  select(game_id, days_since_release, users_rated) %>%
  group_by(game_id) %>%
  mutate(game_horizon_max = max(days_since_release)) %>%
  filter(game_horizon_max >=horizon) %>%
  filter(days_since_release <=horizon) %>%
...
games_release_train %>%
  as.data.frame() %>%
  select(game_id, days_since_release, bayes_average) %>%
  group_by(game_id) %>%
  mutate(game_horizon_max = max(days_since_release)) %>%
  filter(game_horizon_max >=horizon) %>%
  filter(days_since_release <=horizon) %>%
  timetk::tk_augment_lags(., bayes_average, .lags = lag_seq) %>%
  ungroup() %>%
  select(starts_with("bayes_average")) %>%
  cor(use = "pairwise.complete.obs") %>%
  as_tibble() %>%
  mutate(lags = c(0, lag_seq)) %>%
  select(bayes_average, lags) %>%
  filter(lags > 0) %>%
  ggplot(., aes(x=lags, 
         y= bayes_average))+
  geom_point(size=0.5)+
...
This requires us to set up the data with lags for forecasting. If we want to predict user ratings in 7 days from day $i$, or $T_{i+7}$, we can only use information that we have available to us at time $T$. 
