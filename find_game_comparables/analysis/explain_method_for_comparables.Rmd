...
```{r get sample and place, warning=F,fig.height=3, message=F, caption = "Placing a sample of games by complexity"}
...
samp_games_dist %>%
        ...
        bg(., j = names(samp_games_dist[,-1]),
           bg = dist_func) %>%
        set_caption("Distance matrix between games in sample using complexity")


```

So in this small dataset, Pandemic's nearest neighbors are are another cooperative game, followed by some lighter, more family friendly strategy games.
...
samp_games_dist %>%
        ...
        bg(., j = c("dist"),
           bg = dist_func) %>%
        set_caption ("Pandemic's nearest neighbors in sample based on complexity")
...
samp_games_dist %>%
        ...
        bg(., j = c("dist"),
           bg = dist_func) %>%
        set_caption ("Teotihuacan's nearest neighbors in sample based on complexity")
...
We can see the problem we're about to hit if we just try to visualize where every individual game falls on complexity.

```{r placing all games on complexity, fig.height=3, warning=F}

library(ggforce)

set.seed(1)
pos = position_jitternormal(sd_y = 0.025)

jitter_complexity = active_games %>%
        filter(!is.na(avgweight)) %>%
        filter(!is.na(yearpublished)) %>%
...
If you go out to the tails, you might be able to use complexity alone to find some games that are similar. But if you look at a game right in the thick of the distribution, you're going to quickly be overwhelmed and find tons of games.

For instance, you might like Inis. If you look for games that are similar to Inis in complexity, you'll find...

```{r full games comparing on complexity inis, warning=F, message=F}
...
Which, if you know anything about Inis, doesn't seem *quite* right. But the reason these games are deemed similar is that they're all right in a pretty thick part of the distribution.

```{r full games comparing on complexity inis 2, warning=F, message=F}
...
jitter_complexity +
        ...
        geom_label_repel(data = temp,
                   ...
                   size = 3)+
        theme_bw(16)+
        facet_zoom(xlim = c(temp$avgweight-0.3, 
                            temp$avgweight+0.3),
                   horizontal = T)
```

We can do the same thing with a game like War of the Ring, which is a bit higher on the distribution, so it has fewer games next it.
...
But we still end up with a list of games that isn't really what we want. If you're looking at this and going, 'this doesn't feel right, these games aren't that similar', then you've spotted the problem. We've followed the steps we outlined, it sounded well and good, but we've hit a snag at the all important next step:

5. Examine nearest neighbors and evaluate the results.

In other words, we're looking at the output and realizing that it isn't quite mapping to what we want, theoretically.
How do we even know what we want? How are we determining that? Our gut, mostly. Our theoretical concept in our heads of what it means for two games to be similar. If we think about our mental map, we typically think about board concepts like complexity, but also the theme, the mechanics in the game, etc.
...
Let's add an additional dimension on which to compare games. Ideally, we'd like to have answers to our two main questions about a game. 

1) How complex is it?
2) How thematic is it?
...
```{r corrplot, fig.height=10, fig.width=10, caption = "Correlation between all features in dataset"}
...
We'll bake our training set and then run PCA.
...
We can then look to see how much of the variation each principal component explains using what is called a scree plot. In this case, we can see that the first principal component explains about 5% of the overall variation. The first 10 components or so look to explain a little under 25% of the overall variation.
...
To get a sense of what this looks like, we can place every game by their values on the first two principal components.

```{r pca two dimensions plot, fig.height=10, fig.width=10, caption = "Placing all games by first principal components of variation"} 
...
This is a plot of every game in our training set by their values on the first two principal components. What are these two components exactly? Each component is a linear combinations of features in our dataset that maximized the variance while being orthogonal (uncorrelated) with each other. Variables either contribute positively or negatively to each component, and the size of the loading determines how much that variable affects the component.

We can plot the loading of the variables to the first principal component (filtering to the 40 variables with the highest values) to get a sense of what's going on. The first component looks to map to complexity - time per player, average weight, overall playing time, and number of mechanics all increase where your value on this component, while being a party game, a card game, having a higher number of players reduce your value. 
...
We can do the same thing for the second component. A game's value on this component increases with the number of categories the game has (not sure what to make of that just yet), variable player powers, storytelling, role playing, fighting, minatures, adventure, and decreases with if your game is econommic, has networks and trains, tile placement...

This seems to be picking up on that idea of theme, or storytelling. Games that are highly focused on role playing, fighting, adventure, miniatures tend to be of the American school of design that focuses on player interaction to create conflict and stories. In contrast, games focused on economy, tile placement, and route building tend to be of the European school of design, where interaction isn't as important as creating an engaging puzzle. 
...
If we plot the loading scores for these two components, we can see how we get our mapping of games. 

```{r show loadings, warning=F, message=F}
...
Games that are high on the first component (complexity) and the second component (storytelling) end up in the top right. This is where we find highly thematic games like Mansions of Madness, Gloomhaven, Betrayal at House on the Hill, Descent.  

We get a big group of wargames off to the bottom right with games that are highly complex with less of an emphasis on storytelling (I can see one arguing with me on whether this is the right term). The top left is mostly party games and social deduction games, while the bottom left looks to be lighter, abstract family games. 

Games we would think of as Euros look to cluster in the bottom center.

## Finding Distances in Two Dimensions
...
temp =  pca_rotation %>%
                           filter(name == 'The Castles of Burgundy')
...
## Example: Finding Neighbors for **Nemesis**

We can look at a few examples to see what results we get, as well as why we get them. Let's use Nemesis as an example. We have a vector for Nemesis with its value on each of the twenty principal components we are looking at. We can show this using vector by just plotting its value on a line across each component.

```{r line plot for nemesis, warning=F, message=F}
...
What games are the most similar to Nemesis using the three distance metrics? Finding the distance between games is the same as before, but now we're using more dimensions. We'll show the top 25 games from each.
...
nearest = dist_all %>%
        ...
        slice_min(order_by = dist,
                  n = 10,
                  with_ties =F) %>%
        filter(type == 'manhattan') %>%
        pull(closest)

# nearest ids
# names
nearest_ids = dist_all %>%
        filter(grepl(samp, name_id)) %>%
        gather('closest','dist',-name_id, -type) %>%
        mutate(dist = case_when(type == 'cosine' ~ 1-dist,
                                TRUE ~ dist)) %>%
...
background + 
        ...
        geom_label(data  = neighbors_dat,
             ...
             size = 3,
             color = 'blue')+
        facet_zoom(xlim = c(min(neighbors_dat$PC1)-0.1,
                            max(neighbors_dat$PC1)+0.1),
                   ylim = c(min(neighbors_dat$PC2)-0.1,
                             max(neighbors_dat$PC2)+0.1),
                   horizontal = F)+
        theme_bw()
        
```

Why do we find these games as neighbors for Nemesis? We can plot each game's line on the principal components we're looking at. Each one of these components represents a point of variation that is uncorrelated with the other components. They can be difficult to understand, so I've labelled games at the extreme ends of each component on the graph as a reference point.

```{r line plot for nemesis, fig.height=6, fig.width=10, warning=F, message=F}

profile_df = pca_rotation %>%
        select(game_id, name, PC1:PC20) %>%
...
line_plot = profile_df %>%
    ...
    #    theme(legend.title = element_text())+
     #   scale_color_colorblind()+
        ...
        #                    limits = c(-5,5),
        #                    oob = scales::squish)+
        ggtitle(paste("How are games similar to ", nearest[1], "?", sep=""),
...
The basic reason they're considered neighbors? They have similar values and profiles on each of the principal components. By itself, that isn't necessarily helpful in understanding *why* games are similar, given how opaque the principal component scores can be. Let's look at this in further depth by focusing on Nemesis and its neighbors on the first four components.

```{r jitter plots for nd, fig.height=8, fig.width=10, warning=F, message=F}

library(ggforce)
set.seed(2)
pos <- position_jitternormal(sd_x = 0.15, sd_y = 0.05)
pos2 <- position_jitternormal(sd_x = 0.5, sd_y = 0.025)

# get df
references = bind_rows(
        pca_rotation %>%
                select(name, game_id, PC1:PC4) %>%
...
components_background = pca_rotation %>%
        ...
        coord_flip()+
        theme_phil()+
        geom_jitter(data = references,
            aes(x=value,
                y=component),
            alpha =0.3,
            color = 'black',
            position = pos2)+
        geom_label_repel(data = references,
            aes(x=value,
                label = name,
                y=component),
            alpha =0.8,
...
# display
display_nearest = pca_rotation %>%
        filter(name %in% nearest) %>%
        select(name, game_id, PC1:PC4) %>%
        mutate(name = factor(name, levels = nearest)) %>%
        gather("component", "value", -name, -game_id) %>%
        mutate(component = factor(component, levels = paste("PC",
                                                            seq(1,20),
                                                            sep="")))
set.seed(2)
components_background +
        geom_jitter(data = display_nearest,
                    aes(x=value,
                        color = name,
                        y=component),
                    position = pos2)+
        geom_label_repel(data = display_nearest,
                    aes(x=value,
                        label = name,
                        color = name,
                        y=component),
...
scale_color_viridis_d(option="magma",
        ...
        ggtitle(paste("How are games similar to ", nearest[1], "?", sep=""),
            subtitle = str_wrap("Plotting selected games and its neighbors on the first four principal compnents. Similar games will tend to have similar placements along each of the components.", 125))

```

The first component essentially measures complexity, so games that are higher up tend to be more complex. Nemesis is fairly complex, and most of its neighbors (but not all) tend to be higher up here as well. But the most signifying feature for Nemesis and its neighbors seems to be in the second component. These games are all very high on the storytelling/thematic component, which makes sense given what we know about them. 

The games are middle of the pack on the third component, which is mostly a measure of how economic or wargame focused a game is, but they are all low on the fourth component, which I have found to indicate how social/deceptivea a game is.

We can see how certain components define a game pretty easily by using a tile plot.
...
## Placing Well Known Games

The whole purpose of this analysis was to find games that are similar to each other, but with the approach we're taking we can also just place well known games to see how similar/dissimilar they are.

Each game is represented by a vector with its score on each of the principal components. These scores come from multiplying each game's record in the dataset by the 'coefficients' from the principal component loadings. We can grab the top 50 ranked games on BGG and plot their profiles on the first 20 principal components using a tile plot, which will help highlight the similarities and differences between them.

```{r show values for PCS for games, fig.height=10}

set.seed(1)
top_games = pca_rotation %>%
        arrange(desc(baverage)) %>%
        head(50)

top_games %>%
        select(game_id, name, baverage, PC1:PC20) %>%
        gather("component", "value", -name, -game_id, -baverage) %>%
        mutate(component = factor(component, levels = paste("PC", seq(1,20), sep=""))) %>%
        ggplot(., aes(x=component,
                     y=reorder(name,
                               baverage),
...
One thing that jumps out right away is that the top 50 games on the geek list tend have high values on the first principal component (complexity) - this is because the geek ratings tends to skew towards complex games. We also see a lot of these games are high on the 3rd component (economy) and low on the 6th component (I'm not sure). But with some of these similarities in mind, generally speaking their profiles look to be pretty different if we plot them all side by side.

```{r line plot for top games, warning=F, message=F}
...
line_plot = profile_df %>%
        filter(game_id %in% (top_games %>% pull(game_id))) %>%
        #     mutate(name = paste(name, game_id, sep="_")) %>%
        mutate(name = factor(abbreviate(name,
                                        minlength=30),
                             levels = abbreviate(top_games %>% pull(name),
                                                 minlength=30))) %>%
        # mutate(name = factor(abbreviate(name,
        #                                 minlength=30), 
        #                      levels = rev(abbreviate(neighbors[1:7],
...
Let's take Twilight Imperium and plot its values on the principal components.

```{r ti4 line plot, warning=F, message=F}

samp_id = active_games %>% 
        filter(name == 'Twilight Imperium: Fourth Edition') %>%
                    #   name == 'Carcassonne') %>%
        pull(game_id)

game_line_plot = profile_df %>%
        filter(game_id %in% samp_id) %>%
        #     mutate(name = paste(name, game_id, sep="_")) %>%
        # mutate(name = factor(abbreviate(name,
        #                                 minlength=30), 
        #                      levels = rev(abbreviate(neighbors[1:7],
...
Find the game that is least like Twilight Imperium

```{r find dissimillar games, warning=F, message=F}
...
compare = c(samp_id, distant)

game_line_plot = profile_df %>%
        filter(game_id %in% compare) %>%
        #     mutate(name = paste(name, game_id, sep="_")) %>%
        # mutate(name = factor(abbreviate(name,
        #                                 minlength=30), 
        #                      levels = rev(abbreviate(neighbors[1:7],
...
```{r comapre two games, warning=F, message=F}
...
## Placing Dissimilar Games

So, we've looked at one game and its most similar neighbors, and we got mostly similar results regardless of the distance metric.
