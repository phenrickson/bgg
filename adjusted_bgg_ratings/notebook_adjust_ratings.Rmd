...
To do this, we need the *residuals* from a regression of average rating on complexity - these will be the variation in game ratings that are *not explained* by complexity. We will fit the model and inspect the results.
...
complexity_adjusted_ratings %>%
  ...
  select(Published, ID, Game, GeekRank, AdjustedRank, GeekRating, AdjustedRating, Complexity) %>%
  head(250) %>%
        flextable() %>%
        flextable::autofit() %>%
        bg(j = c('Complexity'),
...
## Interactive Table for Complexity Adjusted Ratings
...
```{r interactie DT for complexity ratings, warning=F, message=F}
...
## Analyze and Compare Geek and Complexity Adjusted Ratings
...
## Interactive Table with Different Baseline Votes Added
...
```{r interactive data table, warning=F, message=F}
