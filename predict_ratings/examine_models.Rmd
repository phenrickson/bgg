...
display_preds = function(input_preds,
        ...
        ft_table = table %>%
                ...
                bold(part = "header") %>%
                border_remove() %>%
                bg(., j = c("est_baverage", "baverage"),
                   bg = baverage_func) %>%
                bg(., j = c("est_average", "average"),
                   bg = average_func) %>%
                flextable::align(j = c("est_average", "est_baverage",
                                       "average", "baverage"),
                         align = "center",
                         part = "all")
...
display_preds_with_weights = function(input_preds) {
        ...
        current = input_preds$actual_weight_and_rating %>%
                select(game_id, name, yearpublished, average, baverage, avgweight) %>%
                melt(., id.vars = c("game_id", "name", "yearpublished")) %>%
                rename(outcome = variable) %>%
                mutate(type = "") %>%
                select(yearpublished, game_id, name, type, outcome, value) %>%
        ...
        ft_table = table %>%
                ...
                bg(., j = c("est_average", "average"),
                   bg = average_func) %>%
                bg(., j = c("est_avgweight", "avgweight"),
                   bg = avgweight_func) %>%
                flextable::align(j = c("est_avgweight", "avgweight",
                                       "est_average", "est_baverage",
                                       "average", "baverage"),
                         align = "center",
                         part = "all")
        # add_header(average = "Estimated",
